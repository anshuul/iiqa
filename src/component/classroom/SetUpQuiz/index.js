...
class SetUpQuiz extends Component {
  constructor(props) {
    ...
    this.onCrosshandlerForUploadedImages = this.onCrosshandlerForUploadedImages.bind(
      this
    );
    this.onCrosshandlerForGeneratedQuizImages = this.onCrosshandlerForGeneratedQuizImages.bind(
      this
    );
    ...
    this.togglePredefinedImageSetBox = this.togglePredefinedImageSetBox.bind(
      this
    );
  ...
  fillImagesToBeDisplayed(selectedImageSet) {
    ...
    selectedImageSet.imageLinks.forEach((imageLink) => {
      if (
        !imageSetImages.includes(imageLink) &&
        !generatedQuiz.find((quizObj) => quizObj.image_path === imageLink)
      )
    ...
    if (this.state.isCollectionToBeSaved) {
      ...
      !this.state.loading
        ? this.setState({
            ...this.state,
            loading: true,
            loadingMessage: "Saving your collection first.",
          })
        : this.setState({
            ...this.state,
            loadingMessage: "Saving your collection first.",
  ...
  uploadQuiz() {
    if (this.state.generatedQuiz.length > 0) {
      this.setState({
        ...this.state,
        loading: true,
        loadingMessage: "Uploading your Quiz",
      });
      console.log("quiz name", this.state.quizName);
      createNewQuiz(
        this.state.generatedQuiz,
        this.state.currentClassroomDocId,
        this.state.quizName
      )
        .then((quizDocId) => {
  ...
  setUpQuizHandler() {
    ...
    if (
      this.state.imageSetImages.length === 0 &&
      this.state.uploadedImages.file.length === 0
    ) {
      ...
      this.setState({
        ...this.state,
        loading: true,
        loadingMessage: "Uploading your Chosen Images",
      });
      uploadImagesToFirebaseStorage(this.state.uploadedImages.file).then(
        (uploadedFilesURLs) => {
          console.log(uploadedFilesURLs);
    ...
    reader.onloadend = () => {
      this.setState({
        ...
        uploadedImages: {
          ...
          imagePreviewUrl: [
            ...this.state.uploadedImages.imagePreviewUrl,
            reader.result,
          ],
        },
      });
  ...
  onCrosshandlerForUploadedImages(index) {
    ...
    this.setState({
      ...
      uploadedImages: {
        ...
        imagePreviewUrl: imageURLs.filter(
  ...
  toggleSavedImageSetBox() {
    ...
    const imageContainersHeightValue = this.getHeightOfContainers(
      isSavedBoxOpen,
      isPredfinedBoxOpen
    );
    this.setState({
  ...
  componentDidMount() {
    try {
      if (!this.props.location.state.classroomDocId)
  ...
  render() {
    ...
    return (
      <div
        className="container customQuizMainContainer"
        style={{ height: getHeightForMainContainer() }}
      >
        ...
        <p className="grey-text text-lighten-1">
          Choose images from the given collections, generate quiz and upload it.
        </p>
        <div className="customQuizContainer">
          {/* Image sets container */}
          <div className="customImageMenuContainer">
            <div
              className="customImageSetContainer"
              id="predefined"
              style={{ height: this.state.imageContainersHeightValue }}
            >
              <div
                className="customImageSetContainerTitle"
                onClick={this.togglePredefinedImageSetBox}
              >
                <p>Ready - made</p>
                {this.state.isPredfinedBoxOpen ? (
                  <CollapseIcon />
                ) : (
                  <ExpandIcon />
                )}
            ...
            >
              <div
                className="customImageSetContainerTitle"
                onClick={this.toggleSavedImageSetBox}
              >
                <p>Stored</p>
                {this.state.isSavedBoxOpen ? <CollapseIcon /> : <ExpandIcon />}
              </div>
              {this.state.isSavedBoxOpen && (
                <div className="customImageSetWrapper">
                  {/* list of image sets display */}
          ...
          <div className="customImageViewContainer">
            ...
            <div className="customOptionsContainer">
              ...
              >
                <i
                  ...
                  style={{ fontSize: "1.5rem", top: "1rem", width: 0 }}
                >
                  border_color
                </i>
                <input
            ...
            <div className="customDisplayContainer">
              {this.state.generatedQuiz &&
                this.state.generatedQuiz.map(
                  ({ image_path, question, answer }, index) => (
                    <div
                      className="customImageBlockContainer"
                      key={index}
                      style={{
                    ...
                    >
                      <div
                        ...
                        onClick={() =>
                          this.onCrosshandlerForGeneratedQuizImages(index)
                        }
                      >
              ...
              {[...this.state.uploadedImages.imagePreviewUrl].map(
                (imageURL, index) => (
                  <div
                    className="customImageBlockContainer"
                    key={index}
                  ...
                  >
                    <div
                      className="cancelIcon grey lighten-3"
                      onClick={() =>
                        this.onCrosshandlerForUploadedImages(index)
                      }
                    >
                      <img alt="-" src={MinusIcon} width="12px" />
                    </div>
